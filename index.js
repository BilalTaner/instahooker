const fs = require("fs"), fetch = require("node-fetch"), baseURL = "https://www.instagram.com/",
    ...
    postRegex = /(https?:\/\/(?:www\.)?instagram\.com\/(p|reel|tv)\/([^\/?#&]+)).*/,
...
module.exports.ProfileHooker = async function (settings = {username: "", cookie: "", timeout: 50, output: ""}) {
    const options = {
        "username": settings.username,
        "cookie": `sessionid=${settings.cookie};`,
        "timeout": settings.timeout ? settings.timeout : 50,
        "output": settings.output ? settings.output : "instahooker-out"
    }, posts = [];
    ...
    fetch(`${baseURL}${options.username.startsWith(baseURL) ? profileRegex.exec(options.username)[1] : options.username}/?__a=1`, {headers: {cookie: options.cookie}}).then(async response => {
        const [{graphql}] = await Promise.all([response.json()]);

        if (!graphql) return logger("warn", "User not found!");
        const id = graphql.user.id,
            x = await Promise.all([fetch(`${baseURLQuery}{"id":"${id}","first":"5000"}`, {headers: {cookie: options.cookie}}).then(res => x.json())]),
            mediaCount = x[0].data.user.edge_owner_to_timeline_media.count;

        if (!mediaCount)
            return logger("warn", "There are no posts on the account you entered!");

        let after = x[0].data.user.edge_owner_to_timeline_media.page_info.end_cursor;
        ...
        if (!(mediaCount <= 50)) {
            for (let i = 0; !(i >= Math.floor(mediaCount / 50)); i++) {
                const [fetchingMediaFor] = await Promise.all([fetch(`${baseURLQuery}{"id":"${id}","first":"5000","after":"${after}"}`, {headers: {cookie: options.cookie}})
                    .then(x => x.json())]);
        ...
        if (!(posts.length || !graphql.user.is_private))
...
module.exports.PostHooker = async (settings = { url: "", cookie: "", timeout: 50, output: "" }) => {
  const options = {
      ...
      [data] = await Promise.all([fetch(`${baseURL}/p/${options.url.startsWith(baseURL) ? postRegex.exec(options.url)[3] : options.url}/?__a=1`, {headers: {cookie: options.cookie}})
          .then(res => res.json())]);
...
module.exports.StoryHooker = async (settings = { username: "", cookie: "", timeout: 50, output: "", highlight: false }) => {
  ...
  if (options.highlight) {
    ...
    const hightlight = await fetch(`${baseURLFeed}reels_media/?reel_ids=highlight:${options.username.startsWith("https://www.instagram.com/stories/highlights/") ? highlightRegex.exec(options.username)[1] : options.username}`, headers),
        highlightStories = await hightlight.json();
    ...
    await fetch(`${baseURL}${(!options.username.startsWith(baseURL) ? options.username : profileRegex.exec(options.username)[1])}/?__a=1`, { headers: { cookie: options.cookie } }).then(async response => {
      ...
      await fetch(`${baseURLFeed}reels_tray/`, headers).then(x => x.json()).then(async () => {
        ...
        await logger("fetchReady");
        const [stories] = await Promise.all([fetch(`${baseURLFeed}user/${id}/reel_media/`, headers).then(res => res.json())]);

        getData(stories, posts, "story");

        //Checking array for story is exits
        if (!posts.length) {
          return data.graphql.user.is_private ? logger("warn", "The account you want to reach is private and you don't follow it, so media cannot be downloaded!") : logger("warn", "There are no stories on the account you entered!");
        }
